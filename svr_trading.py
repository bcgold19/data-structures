# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 20:17:10 2021
...
def smart_beta_svr(new_ex_return,new_factor,tickers,target,window,rolling_window,start,end='2021-11-19'):
    ...
    weight_matrix = pd.DataFrame(index=p_dates, columns=tickers)
    ...
    support_vector_residual=pd.DataFrame(index=p_dates,columns=tickers)
    ...
    for i in range(len(p_dates)):
        if(p_dates[i]==rebalancing_date):
            ...
            SVRs,sv_predict,residual,linear_reg=hybrid_SVR_entire(new_factor_norm[target1 & target2],new_ex_return[target1 & target2],rolling_window)
            #print(residual)
            mu_f=linear_reg[0]
            sigma_f=linear_reg[1]
            B=linear_reg[2]
            alpha=linear_reg[3]
            Psi=linear_reg[4]
            ret_cov=B@sigma_f@B.T+Psi+np.cov(residual)
            p_mu=alpha+B@mu_f+np.mean(residual,axis=1)
            #ret_cov=np.cov(sv_predict)
            #p_mu=np.mean(sv_predict,axis=1)
            n=ret_cov.shape[1]
            print(ret_cov.shape)
            print(sv_predict.shape)
            ...
            if(target=='mvs'):
                ...
                weight_matrix.loc[p_dates[i]]=error_signal_weight(residual,tickers)
            else:
